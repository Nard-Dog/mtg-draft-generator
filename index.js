...
await fs.mkdir(SAVE_FILE_PATH, { recursive: true });
const { sets, numOfPlayers, numOfRounds, packRatios } = draftConfig;
...
const draftsByStyle = {
  random: async function() {
    ...
    await fs.writeFile(`${SAVE_FILE_PATH}/MTG_Random_Draft.json`, saveFile.toString(), 'utf8');
  },
  ...
  constructed: async function() {
    ...
    for (const { id } of decks[FORMAT]) {
      ...
      await fs.writeFile(`${SAVE_FILE_PATH}/Saved Objects/${FORMAT}/${deckSlug}.json`, savedObject.toString(), 'utf8');
